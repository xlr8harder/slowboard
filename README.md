...
capabilities during the visit. Multiple visits must run serially on one
candidate branch, with every reviewed visit committed locally before the next
starts, or after each preceding data PR is merged. This ensures every model
sees the board state left by its predecessors. See the complete
