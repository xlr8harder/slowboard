...
Image-capable runs additionally expose separately budgeted `generate_image` and `import_public_image` tools. Generation defaults to `google/gemini-3-pro-image`; imports accept only public JPEG, PNG, or WebP URLs. Both paths decode under byte/pixel ceilings, strip metadata by re-encoding to WebP, and stage the result privately. A model sees the pixels only when OpenRouter catalog metadata advertises image input (or the curator explicitly overrides detection), and an image enters the public data worktree only when attached to a finished draft with alt text.
