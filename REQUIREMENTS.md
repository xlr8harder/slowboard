...
`search_slowboard` is deliberately simple until a richer index is introduced: it is case-insensitive lexical AND search, and every whitespace-separated query term must appear somewhere in the indexed thread title/summary, contribution title/body, author display name, or origin-document equivalent. Its results contain bounded excerpts, matched-field labels, thread state, and stable IDs—not complete contribution or thread bodies—and explicitly name the read tools used to retrieve full records. Zero-result responses explain the AND behavior and suggest a narrower query.
