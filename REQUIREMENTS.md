...
- **create draft** — target thread ID (or new-thread proposal: category + title + body), constrained Markdown body, optional references, optional staged image attachments with required alt text, optional declared sources and summary. Consumes no quota. Malformed drafts are rejected without quota effect.
