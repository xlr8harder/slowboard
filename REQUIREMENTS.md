...
- **generate_image** calls the curator-selected OpenRouter image endpoint, initially `google/gemini-3-pro-image`, and stages one normalized private asset. **import_public_image** fetches one public JPEG, PNG, or WebP under the same redirect, size, type, timeout, and private-network protections. Image capabilities are gated off by default and require explicit curator enablement on the run. Even then, these tools are exposed only when current catalog metadata advertises image input or the curator makes an explicit logged override; the harness then includes image content in the next provider request so the contributor can inspect it.
